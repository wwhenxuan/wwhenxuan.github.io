...
由于我们将预测看作是Next-token prediction任务，因此在下一个时间戳位置上进行预测时，将会根据历史的输入（前几个Token）确定当前时刻最有可能的那个Token（Softmax概率预测），具体的预测任务可以写为$$p(z_{C+h+1}|z_{1:C+h})$$，其中$$z_{C+h+1}$$表示要预测的下一个Token，
其中$$z_{1:C+h}$$表示历史的输入信息。因此该训练方式需要使用交叉熵损失函数：
...
该模型在实验中并未与大量的深度学习模型进行比较，仅与一些传统的预测模型例如ARMA、ARIMA和AutoARIMA等，并且实验的数据大不分也仅限于一些传统的时间序列比如自回归训练等。

![image-20250305213248192](..\images\blogs\2025-3-5-Chronos\results.png)
